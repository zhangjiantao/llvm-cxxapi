...
struct CxxApiWriterPass : public ModulePass {
  static char ID;
  explicit CxxApiWriterPass(raw_ostream &OS, std::string InFile, bool IR,
                            bool Short)
      : ModulePass(ID), Out(OS), InputFile(std::move(InFile)), isPrintIR(IR),
        isGenShortName(Short), TheModule(nullptr), DefNames(Short),
        IndentLevel(0) {}
...
bool CxxApiWriterPass::runOnModule(Module &M) {
  ...
  InputFile = InputFile == "-" ? "<stdin>" : InputFile;
  ...
  nl() << "// Generate Command:";
  nl() << "//";
  nl() << "//   llvm-cxxapi -ir=" << btostr(isPrintIR)
       << " -short=" << btostr(isGenShortName) << " " << InputFile;
  nl() << "//";
  nl();
...
ModulePass *createCxxApiWriterPass(raw_ostream &OS, const std::string &InFile,
                                   bool IR, bool Short) {
  return new CxxApiWriterPass(OS, InFile, IR, Short);
}
} // namespace llvm
