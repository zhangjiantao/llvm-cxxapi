...
class NameBuilder {
  ...
  std::string getPrefix(const Value *Val) {
    ...
    if (auto G = dyn_cast<GlobalVariable>(Val)) {
      ...
      return "CExpr" + getPrefix(Val->getType());
    } else if (isa<Function>(Val)) {
      return "Func" + Normalize(Val->getName().str());
    } else if (auto C = dyn_cast<Constant>(Val)) {
      return "Const" + getPrefix(C->getType());
    } else if (auto Ins = dyn_cast<Instruction>(Val)) {
      if (auto GEP = dyn_cast<GetElementPtrInst>(Val))
        return "GEP" + getPrefix(GEP->getResultElementType());
      if (auto Alloc = dyn_cast<AllocaInst>(Val))
        return "Alloc" + getPrefix(Alloc->getAllocatedType());
      return Normalize(Ins->getOpcodeName()) + getPrefix(Val->getType());
    } else {
